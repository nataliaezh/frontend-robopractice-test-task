...
const MyTable = ({searchQuery}) => {
    ...
    const getUserMonthActivity = (user) => {
      var userMonthActivity = new Array(31);
      var differenses = monthDays.filter(x => getWorkedDays(user).indexOf(x) === -1)

      for(var i = 1; i <= userMonthActivity.length; i++) {
        ...
        if(differenses.includes(i)) {
          ...
          var start = user.Days.find((day) => new Date(day.Date).getDate() === i).Start
          var end = user.Days.find((day) => new Date(day.Date).getDate() === i).End
    ...
    const calculateTime = (start,end) => {
      var endMinutes = getMinutesFromDate(end);
      var startMinutes = getMinutesFromDate(start);

      var delta  = endMinutes - startMinutes;

      var resultHours = addInsignificantZero(String(Math.floor(delta/60)));
      var resultMinutes = addInsignificantZero(String(delta % 60));
    ...
    const getTotalTime = (days) => {
      ...
      var totalHours = addInsignificantZero(String(Math.floor(total/60)));
      var totalMinutes = addInsignificantZero(String(total % 60));
